...
class model_wxconnect_uc
{
    // 自动注册
    public function regist($nickname)
    {/*{{{*/
        $nickname = $this->clear($nickname);
        $username = substr($nickname, 0, 15);
        $password = '213456'; //md5(random(10));
        $email = 'wxconnect_'.strtolower(random(10)).'@null.null';
        return $this->_regist($username, $password, $email);
    }/*}}}*/
    ...
    // 生成随机密码
    public function gen_rand_password()
    {/*{{{*/
    ...
    // 注册
    public function _regist($username, $password, $email, $profile=array())
    {/*{{{*/
        global $_G;
        require_once libfile('function/misc');
        require_once libfile('function/mail');
		...
		try {
			...
			if( !empty($user)) {
				$username = substr($username, 0, 7).'_'.random(5);
			}
    ...
    // 同步用户头像
    public function sync_avatar($uid, $headimgurl)
    {/*{{{*/
        if(!$uid || !$headimgurl) {
            return false;
        }
        if(!$content = dfsockopen($headimgurl)) {
            return false;
        }   
        ...
        foreach (array(1,2,3) as $i) {
            if (wxconnect_uploadUcAvatar::upload($uid, $tmpFile)) {
                break;
            } else {
                wxconnect_env::getlog()->warning("upload_avatar_fail: [uid:$uid] [imgurl:$headimgurl]");
            }
        }
        unlink($tmpFile);
...
// 从wechat/wechat.class.php文件复制
class wxconnect_uploadUcAvatar 
{/*{{{*/
